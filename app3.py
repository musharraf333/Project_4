...
def main():
    ...
    st.write('Welcome to the COMC Seller Stats Breakdown app. This was created by Musharraf Abbasi (Breakout Cards / Ryan Nolan Data on YouTube)')
    ...
    st.write('Looking to have some data analyzed for your business? Or Need help with a Data Project? I take on Freelance work. Email me at musharrafabbasi282@gmail.com (Also open to full time job opportunities)')

    # Upload file
    files = st.file_uploader("Upload multiple spreadsheets (CSV files only)", type=["csv"], accept_multiple_files=True)
    ...
    if files:
        ...
        # Combine all DataFrames into one and assign it to df
        ...
        with col6:
            ...
            comc_count = (~filtered_df['Purchased By'].isin(['eBay', '*Cart*'])).sum()  # Assuming anything other than eBay or Cart is Comc
